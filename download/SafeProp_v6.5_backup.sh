#!/system/bin/sh
# ============================================================
# SafeProp v6.5 — Disguise HP jadi HP lain
...
if [ -f "$LOCKFILE" ]; then
    # Stale check — kalau lock >5 menit, hapus
...
# --- Warna (Hacker palette — clean, no basic colors) ---
G='\e[0;32m'
BG='\e[01;32m'
C='\e[0;36m'
W='\e[0;37m'
...
# --- Mode pesawat (NUCLEAR) ---
# 3 Layer jaminan semua jaringan MATI:
#   Layer 1: Radio off per-interface (WiFi/BT/Cellular)
#   Layer 2: Hapus semua routing table
#   Layer 3: iptables DROP semua output (firewall total)
# Semua auto-reset saat reboot — gak ada efek samping
#
# Android 11 crDroid lavender test result:
#   ip link set wlan0 down → BEKERJA
#   svc bluetooth disable → BEKERJA
#   service call phone 3 i32 0 → BEKERJA (RIL radio off, return 1=success)
#   svc wifi/data disable → GAGAL
#   settings put global mobile_data 0 → GAGAL
modpeson(){
    # ===== KILL VPN DULU =====
    # Kill Switch bisa block radio control
    ...
    # ===== LAYER 1: RADIO OFF =====
    # WiFi — ip link (terbukti bekerja)
    ip link set wlan0 down 2>/dev/null

    # Bluetooth — svc (terbukti bekerja)
    svc bluetooth disable 2>/dev/null

    # Cellular — RIL radio off via ITelephony (return 1 = success)
    service call phone 3 i32 0 2>/dev/null

    # Cellular — backup: down semua interface rmnet
    ...
    # Hapus semua route — gak ada jalur keluar
    ip route flush table all 2>/dev/null
    ip route flush cache 2>/dev/null

    # ===== LAYER 3: FIREWALL TOTAL =====
    # iptables DROP semua output — jaminan 100% gak ada koneksi keluar
    # Semua rule auto-hapus saat reboot
    iptables -P OUTPUT DROP 2>/dev/null
    ip6tables -P OUTPUT DROP 2>/dev/null

    # Update setting value (UI indicator)
    settings put global airplane_mode_on 1 2>/dev/null
}

modpesoff(){
    # Setelah reboot, Android auto-restore semua:
    # - iptables/ip6tables reset ke default ACCEPT
    # - Routing table rebuild oleh Netd
    # - WiFi/BT/Data nyala sesuai saved config
    # - Cellular radio auto-on oleh RIL
    # Jadi gak perlu restore manual apa-apa
}

# --- Progress bar (classic 1-line, width=30) ---
progress() {
    pct="$1"; msg="$2"
    w=30
    filled=$((pct * w / 100))
...
# --- Bersih — satu fungsi untuk semua ---
# Prinsip: STOP semua dulu, baru CLEAR semua (2-pass)
# Mencegah race condition app menulis data saat di-clear
bersih(){

    # ========== PASS 1: STOP SEMUA ==========

    # Kill telephony process (aman — system_server auto-restart setelah reboot)
    am kill com.android.phone 2>/dev/null
    am force-stop com.android.phone 2>/dev/null

    # Stop semua tracking app
    for pkg in \
    ...
    # Stop WebView (setelah browser sudah di-stop)
    am force-stop com.android.webview 2>/dev/null

    # Stop system component — yang akan di-pm clear
    # Note: ext.services/ext.shared bisa android.* atau com.android.* tergantung ROM
    for pkg in \
    ...
    # Clear WebView (aman karena semua browser sudah di-stop & di-clear)
    pm clear com.android.webview 2>/dev/null

    # Clear system component (data auto-regenerate saat boot)
    # Note: ext.services/ext.shared bisa android.* atau com.android.* tergantung ROM
    for pkg in \
        com.android.location.fused \
        com.google.android.overlay.gmsconfig.common \
...
Vivo Y53s|vivo|vivo|V2058|PD2103F|PD2103F|vivo/PD2103F/PD2103F:11/RP1A.200720.011/compilation:user/release-keys|2021-06-01|mt6765|mt6833
...
RedMagic 6|nubia|nubia|NX669J|NX669J|NX669J_GL|nubia/NX669J_GL/NX669J:11/RKQ1.210608.001/V3.08:user/release-keys|2021-06-01|qcom"
    ...
    # Random pick — pakai /dev/urandom (bukan $RANDOM)
    TOTAL=$(printf '%s\n' "$MODEL_DB" | wc -l)
    RNUM=$(rand_num "$TOTAL")
    RNUM=$((RNUM + 1))
    pick=$(printf '%s\n' "$MODEL_DB" | sed -n "${RNUM}p")

    # Fallback kalau pick kosong
    ...
    cat > "$dir" << PROPS
# SafeProp v6.5 — Generated $(date +%Y-%m-%d)
# Device: $display_name
...
nuketrack(){
    ...
    rm -f /data/system/users/0/settings_config.xml
    rm -f /data/system/users/0/app_idle_stats.xml
...
printf '\n'
printf '%b' "${C}       M A U N G   P R O T O C O L${N} ${W}v6.5${N}\n"
printf '%b' "${W}  ----------------------------------------------${N}\n"
...
# Step 4: Acak serial
printf '  %b>%b %bSCRAMBLE%b %bRandomize serial%b\n' "$BG" "$N" "$C" "$N" "$W" "$N"
progress 0 "Scrambling..."
acakserial >/dev/null 2>&1
anim_bar 0 100 "Serial scrambled"
printf '\n'
